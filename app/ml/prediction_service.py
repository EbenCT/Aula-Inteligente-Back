...
class PredictionService:
    def __init__(self, models_path: str = "modelos"):
        """
    ...
    def predecir_rendimiento(self, datos: Dict) -> Dict:
        ...
        prediccion_numerica = np.clip(prediccion_numerica, 0, 100)  # Asegurar rango válido
        
        # Predicción categórica (clasificación)
        prediccion_categoria_encoded = self.modelo_clasificacion.predict(X_scaled)[0]
        prediccion_categoria = self.label_encoder.inverse_transform([prediccion_categoria_encoded])[0]
        
        # Probabilidades por clase
        probabilidades = self.modelo_clasificacion.predict_proba(X_scaled)[0]
        probabilidades_dict = dict(zip(
            self.label_encoder.classes_, 
            [float(p) for p in probabilidades]
        ))
        ...
        recomendaciones = self.generar_recomendaciones(datos, prediccion_numerica, prediccion_categoria)
    ...
    def generar_recomendaciones(self, datos: Dict, prediccion: float, categoria: str) -> List[str]:
        """
        ...
        if prediccion < 50:
            ...
            recomendaciones.append("📖 Considerar apoyo adicional en materias específicas")
        else:
    ...
    def predecir_estudiante_por_bd(self, db, estudiante_id: int, materia_id: int, periodo_id: int) -> Dict:
        """
        ...
        try:
            ...
            if not result:
                raise ValueError(f"No se encontraron datos para estudiante {estudiante_id}")
    ...
    def predecir_lote_estudiantes(self, db, curso_id: int, materia_id: int, periodo_id: int) -> List[Dict]:
        """
        ...
        try:
            ...
            estudiantes = db.execute(text(query_estudiantes), {'curso_id': curso_id}).fetchall()
            ...
            for estudiante_row in estudiantes:
                try:
                    ...
                    logger.warning(f"Error prediciendo estudiante {estudiante_row.estudiante_id}: {str(e)}")
                    continue
    ...
    def obtener_estudiantes_en_riesgo(self, db, umbral_riesgo: float = 60.0, 
        ...
        try:
            ...
            for estudiante in estudiantes:
                try:
                    ...
                    materia_result = db.execute(text(query_materia), 
                                              {'curso_id': estudiante.curso_id}).fetchone()
                    
                    if not materia_result:
                        continue
                    
                    prediccion = self.predecir_estudiante_por_bd(
                        db, estudiante.estudiante_id, 
                        materia_result.materia_id, estudiante.periodo_actual
                    )
                    
                    # Verificar si está en riesgo
                    if (prediccion['prediccion_numerica'] < umbral_riesgo or 
                        prediccion['nivel_riesgo'] in ['alto', 'critico']):
                        ...
                        estudiantes_riesgo.append({
                            ...
                            'recomendaciones': prediccion['recomendaciones'][:3]  # Solo primeras 3
                        })
                    ...
                    logger.warning(f"Error evaluando riesgo para estudiante {estudiante.estudiante_id}: {str(e)}")
                    continue
            
            # Ordenar por nivel de riesgo y predicción
            orden_riesgo = {'critico': 4, 'alto': 3, 'medio': 2, 'bajo': 1}
            estudiantes_riesgo.sort(
                key=lambda x: (orden_riesgo.get(x['nivel_riesgo'], 0), -x['prediccion_numerica'])
            )
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if not service.models_loaded:
                raise RuntimeError("Servicio de predicción no disponible. Modelos no cargados.")
...
def crear_prediccion_response(prediccion: Dict) -> Dict:
    ...
    return PredictionResponse(
        ...
        data={
            ...
            'interpretacion': {
                'nivel': 'Excelente' if prediccion['prediccion_numerica'] >= 80 
                        else 'Bueno' if prediccion['prediccion_numerica'] >= 70
                        else 'Regular' if prediccion['prediccion_numerica'] >= 60
                        else 'Necesita Mejora',
                'color': 'success' if prediccion['prediccion_numerica'] >= 70
                        else 'warning' if prediccion['prediccion_numerica'] >= 60
                        else 'danger',
                'icono': '🟢' if prediccion['prediccion_numerica'] >= 70
                        else '🟡' if prediccion['prediccion_numerica'] >= 60
                        else '🔴'
            }
        }
    ).to_dict()
