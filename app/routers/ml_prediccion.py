...
class PredictionRequest(BaseModel):
    ...
    @validator('promedio_notas_anterior', 'porcentaje_asistencia', 'promedio_participacion')
...
def predecir_curso_completo(
    ...
    try:
        ...
        promedio_curso = sum(p['prediccion_numerica'] for p in predicciones) / len(predicciones)
        ...
        raise HTTPException(status_code=500, detail="Error interno en predicción del curso")
...
def obtener_estudiantes_riesgo(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Error obteniendo estudiantes en riesgo")
...
def obtener_estadisticas_modelo(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Error obteniendo estadísticas del modelo")
        ...
        return {
            ...
            "data": {
                ...
                "modelo_info": {
                    ...
                    "clasificacion": stats.get('modelo_clasificacion', {}).get('tipo', 'N/A')
                },
    ...
    try:
        ...
        for row in resultados[:5]:  # Limitar para evitar sobrecarga
            try:
                ...
                logger.warning(f"Error prediciendo estudiante {row.estudiante_id}: {str(e)}")
                continue
        ...
        raise HTTPException(status_code=500, detail="Error obteniendo predicciones recientes")
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "acciones_sugeridas": {
                    "inmediatas": [rec for rec in resultado['recomendaciones'] if "🚨" in rec or "⚠️" in rec],
                    "seguimiento": [rec for rec in resultado['recomendaciones'] if "📚" in rec or "📖" in rec],
                    "motivacion": [rec for rec in resultado['recomendaciones'] if "✅" in rec or "🌟" in rec]
                }
            }
        }
        
    except Exception as e:
        logger.error(f"Error obteniendo recomendaciones para estudiante {estudiante_id}: {str(e)}")
...
def predicciones_materias_docente(
    ...
    try:
        ...
        materias = db.execute(text(query_materias), {'docente_id': docente_id}).fetchall()
        
        if not materias:
            raise HTTPException(status_code=404, detail="No se encontraron materias para este docente")
        ...
        for materia in materias:
            try:
                ...
                cursos = db.execute(text(query_cursos), {'materia_id': materia.materia_id}).fetchall()
                ...
                for curso in cursos:
                    try:
                        ...
                        if predicciones_curso:
                            # Calcular estadísticas del curso
                            promedio_curso = sum(p['prediccion_numerica'] for p in predicciones_curso) / len(predicciones_curso)
                            estudiantes_riesgo = [p for p in predicciones_curso if p['nivel_riesgo'] in ['alto', 'critico']]
                            ...
                            predicciones_materia.append({
                                ...
                                "predicciones": predicciones_curso[:5]  # Solo primeros 5 para resumen
                            })
                    
                    except Exception as e:
                        logger.warning(f"Error prediciendo curso {curso.curso_id}: {str(e)}")
                        continue
                
                if predicciones_materia:
                    # Calcular estadísticas generales de la materia
                    total_estudiantes = sum(p['total_estudiantes'] for p in predicciones_materia)
                    promedio_general = sum(p['promedio_curso'] * p['total_estudiantes'] for p in predicciones_materia) / total_estudiantes if total_estudiantes > 0 else 0
                    total_riesgo = sum(p['estudiantes_riesgo'] for p in predicciones_materia)
                    ...
                    resultados_por_materia.append({
                        ...
                        "porcentaje_riesgo": round((total_riesgo / total_estudiantes) * 100, 1) if total_estudiantes > 0 else 0,
                        "cursos": predicciones_materia
                    })
            
            except Exception as e:
                logger.warning(f"Error procesando materia {materia.materia_id}: {str(e)}")
                continue
        ...
        logger.error(f"Error obteniendo predicciones para docente {docente_id}: {str(e)}")
        raise HTTPException(status_code=500, detail="Error obteniendo predicciones del docente")
...
def analisis_completo_curso_materia(
    ...
    try:
        ...
        predicciones = service.predecir_lote_estudiantes(db, curso_id, materia_id, periodo_id)
        
        if not predicciones:
            raise HTTPException(status_code=404, detail="No se encontraron estudiantes para analizar")
        ...
        estadisticas = {
            ...
            "desviacion_estandar": round((sum((x - sum(notas_predichas)/len(notas_predichas))**2 for x in notas_predichas) / len(notas_predichas))**0.5, 2)
        }
        ...
        if len(atencion_inmediata) > len(predicciones) * 0.3:  # Más del 30% en riesgo
            recomendaciones_curso.append("🚨 Alto porcentaje de estudiantes en riesgo - Revisar metodología")
            recomendaciones_curso.append("📚 Implementar programa de refuerzo académico grupal")
        
        if estadisticas['promedio'] < 65:
            recomendaciones_curso.append("📈 Promedio curso bajo - Evaluar dificultad del contenido")
            recomendaciones_curso.append("🔄 Considerar ajustes en plan de estudios")
        
        if estadisticas['desviacion_estandar'] > 20:
            recomendaciones_curso.append("📊 Alta dispersión en rendimiento - Atención individualizada")
        
        if len(alto_rendimiento) > 0:
            recomendaciones_curso.append(f"🌟 {len(alto_rendimiento)} estudiantes destacados - Oportunidades avanzadas")
        ...
        return {
            ...
            "data": {
                ...
                "estudiantes_atencion_inmediata": {
                    ...
                    "porcentaje": round((len(atencion_inmediata) / len(predicciones)) * 100, 1),
                    "estudiantes": atencion_inmediata
                },
                "estudiantes_alto_rendimiento": {
                    "total": len(alto_rendimiento),
                    "porcentaje": round((len(alto_rendimiento) / len(predicciones)) * 100, 1),
        ...
        logger.error(f"Error en análisis completo curso {curso_id} materia {materia_id}: {str(e)}")
...
def reporte_resumen_institucional(
    ...
    try:
        ...
        if payload.get("is_doc") != False:
            raise HTTPException(status_code=403, detail="Solo administradores pueden ver reportes institucionales")
        ...
        if not predicciones_muestra:
            raise HTTPException(status_code=404, detail="No se pudieron generar predicciones para el reporte")
        
        # Calcular métricas institucionales
        total_muestra = len(predicciones_muestra)
        promedio_institucional = sum(p['prediccion_numerica'] for p in predicciones_muestra) / total_muestra
        ...
        for nivel in por_nivel:
            if por_nivel[nivel]['total'] > 0:
                por_nivel[nivel]['promedio'] = round(por_nivel[nivel]['promedio'] / por_nivel[nivel]['total'], 2)
                por_nivel[nivel]['porcentaje_riesgo'] = round((por_nivel[nivel]['riesgo'] / por_nivel[nivel]['total']) * 100, 1)
        ...
        estudiantes_riesgo_total = len([p for p in predicciones_muestra if p['nivel_riesgo'] in ['alto', 'critico']])
        porcentaje_riesgo = (estudiantes_riesgo_total / total_muestra) * 100
        
        if porcentaje_riesgo > 25:
            alertas.append(f"🚨 {porcentaje_riesgo:.1f}% de estudiantes en riesgo académico")
        ...
        raise HTTPException(status_code=500, detail="Error generando reporte institucional")
...
try:
    ...
    class MockPandas:
        class Timestamp:
            @staticmethod
            def now():
                return type('MockTimestamp', (), {'isoformat': lambda: datetime.now().isoformat()})()
