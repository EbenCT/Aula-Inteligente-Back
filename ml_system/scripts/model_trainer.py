...
class ModelTrainerAjustado:
    ...
    def cargar_y_validar_datos(self):
        ...
        try:
            ...
            columnas_requeridas = [
                ...
                'promedio_participacion', 'nota_final_real', 'rendimiento_categoria'
            ]
            ...
            print(f"   📈 Rango notas: {self.df['nota_final_real'].min():.1f} - {self.df['nota_final_real'].max():.1f}")
            ...
            for cat, count in self.df['rendimiento_categoria'].value_counts().items():
    ...
    def analisis_exploratorio_realista(self):
        ...
        estadisticas = self.df[['promedio_notas_anterior', 'porcentaje_asistencia', 
                              'promedio_participacion', 'nota_final_real']].describe()
        print(estadisticas.round(2))
        
        # Correlaciones
        print(f"\n🔗 Matriz de correlaciones:")
        correlaciones = self.df[['promedio_notas_anterior', 'porcentaje_asistencia', 
                               'promedio_participacion', 'nota_final_real']].corr()
        ...
        print(f"\n📚 Análisis por materia:")
        por_materia = self.df.groupby('materia_nombre')['nota_final_real'].agg(['count', 'mean', 'std']).round(2)
        print(por_materia)
        
        # Análisis temporal (por período)
        print(f"\n📅 Análisis por período:")
        por_periodo = self.df.groupby('periodo_nombre')['nota_final_real'].agg(['count', 'mean', 'std']).round(2)
        print(por_periodo)
    ...
    def crear_visualizaciones_reales(self):
        ...
        self.df['nota_final_real'].hist(bins=20, ax=axes[0,0], alpha=0.7, color='skyblue', edgecolor='black')
        axes[0,0].set_title('Distribución de Notas Finales\n(Datos Reales)', fontsize=12, fontweight='bold')
        axes[0,0].set_xlabel('Nota Final')
        axes[0,0].set_ylabel('Frecuencia')
        axes[0,0].axvline(self.df['nota_final_real'].mean(), color='red', linestyle='--', 
                         label=f'Media: {self.df["nota_final_real"].mean():.1f}')
        axes[0,0].legend()
        
        # 2. Boxplot por categoría
        categorias = ['Bajo', 'Medio', 'Alto']
        datos_boxplot = [self.df[self.df['rendimiento_categoria']==cat]['nota_final_real'] 
                        for cat in categorias if cat in self.df['rendimiento_categoria'].values]
        labels_boxplot = [cat for cat in categorias if cat in self.df['rendimiento_categoria'].values]
        
        axes[0,1].boxplot(datos_boxplot, labels=labels_boxplot)
        axes[0,1].set_title('Distribución por Categoría de Rendimiento', fontsize=12, fontweight='bold')
        axes[0,1].set_ylabel('Nota Final')
        
        # 3. Correlación entre variables
        correlacion_matrix = self.df[['promedio_notas_anterior', 'porcentaje_asistencia', 
                                    'promedio_participacion', 'nota_final_real']].corr()
        sns.heatmap(correlacion_matrix, annot=True, cmap='RdYlBu_r', center=0, ax=axes[0,2],
                   square=True, fmt='.3f')
        axes[0,2].set_title('Matriz de Correlaciones', fontsize=12, fontweight='bold')
        
        # 4. Scatter plot principal
        scatter = axes[1,0].scatter(self.df['promedio_notas_anterior'], self.df['nota_final_real'], 
                                   c=self.df['porcentaje_asistencia'], cmap='viridis', alpha=0.6)
        axes[1,0].set_xlabel('Promedio Notas Anterior')
        axes[1,0].set_ylabel('Nota Final Real')
        axes[1,0].set_title('Relación: Promedio Anterior vs Nota Final\n(Color = Asistencia)', 
        ...
        axes[1,1].bar(range(len(materias_top)), materias_top.values, color='lightcoral')
        axes[1,1].set_title('Registros por Materia (Top 5)', fontsize=12, fontweight='bold')
        ...
        axes[1,1].set_xticklabels([name[:15] + '...' if len(name) > 15 else name 
                                  for name in materias_top.index], rotation=45)
        
        # 6. Distribución de asistencia
        self.df['porcentaje_asistencia'].hist(bins=15, ax=axes[1,2], alpha=0.7, color='lightgreen', edgecolor='black')
        axes[1,2].set_title('Distribución de Asistencia', fontsize=12, fontweight='bold')
        axes[1,2].set_xlabel('Porcentaje de Asistencia')
        axes[1,2].set_ylabel('Frecuencia')
        axes[1,2].axvline(self.df['porcentaje_asistencia'].mean(), color='red', linestyle='--',
    ...
    def preparar_features_optimizadas(self):
        ...
        features_adicionales = [
            ...
            'promedio_practicas', 'edad', 'genero_masculino', 'turno_manana'
        ]
        
        for feature in features_adicionales:
            if feature in self.df.columns and self.df[feature].notna().sum() > len(self.df) * 0.5:
        ...
        self.y_clasificacion_encoded = self.label_encoder.fit_transform(self.y_clasificacion)
        
        print(f"✅ Datos preparados: {self.X.shape[0]} muestras, {self.X.shape[1]} características")
        print(f"   📊 Rango y_regresion: {self.y_regresion.min():.1f} - {self.y_regresion.max():.1f}")
        print(f"   📊 Clases y_clasificacion: {list(self.label_encoder.classes_)}")
    ...
    def entrenar_modelos_ajustados(self):
        ...
        X_train, X_test, y_reg_train, y_reg_test, y_clf_train, y_clf_test = train_test_split(
            self.X_scaled, self.y_regresion, self.y_clasificacion_encoded, 
            test_size=0.25, random_state=42, stratify=self.y_clasificacion_encoded
        )
        ...
        # Validación cruzada
        cv_scores_r2 = cross_val_score(rf_regressor, self.X_scaled, self.y_regresion, 
                                      cv=5, scoring='r2')
        print(f"   📊 Validación cruzada R²: {cv_scores_r2.mean():.4f} ± {cv_scores_r2.std():.4f}")
        ...
        # Validación cruzada
        cv_scores_acc = cross_val_score(rf_classifier, self.X_scaled, self.y_clasificacion_encoded, 
                                       cv=5, scoring='accuracy')
        print(f"   📊 Validación cruzada Accuracy: {cv_scores_acc.mean():.4f} ± {cv_scores_acc.std():.4f}")
        
        self.modelo_clasificacion = rf_classifier
        
        # Importancia de características
        self.feature_importance = {
            'regresion': dict(zip(self.features_principales, rf_regressor.feature_importances_)),
            'clasificacion': dict(zip(self.features_principales, rf_classifier.feature_importances_))
        }
    ...
    def guardar_modelos_y_metadatos(self):
        ...
        metadatos = {
            ...
            'clases_disponibles': list(self.label_encoder.classes_),
            'distribucion_clases': dict(self.df['rendimiento_categoria'].value_counts())
        }
...
- Error RMSE: {rmse:.4f}
- Modelo Clasificación (Accuracy): {accuracy:.4f}

EVALUACIÓN DE CALIDAD:
- R² > 0.60: {'✅ Bueno' if r2 > 0.60 else '⚠️ Mejorable'}
- RMSE < 20: {'✅ Aceptable' if rmse < 20 else '⚠️ Alto error'}
- Accuracy > 0.70: {'✅ Buena clasificación' if accuracy > 0.70 else '⚠️ Revisar datos'}
...
"""
    ...
    def ejecutar_entrenamiento_completo(self):
        ...
        print("🚀 ENTRENAMIENTO CON DATOS REALES - 210 ESTUDIANTES")
        print("=" * 60)
        ...
        print(f"🎯 R² Score: {r2:.3f} | RMSE: {rmse:.2f} | Accuracy: {accuracy:.3f}")
        print(f"📁 Archivos guardados en directorio 'modelos/'")
        
        print(f"\n📋 PRÓXIMOS PASOS:")
        print(f"1. ✅ Modelos entrenados y guardados")
        print(f"2. 🔄 Integrar con FastAPI (prediction_service.py)")
        print(f"3. 🧪 Probar predicciones con datos reales")
        print(f"4. 🚀 Preparar demo para presentación")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print(f"Los modelos están optimizados para tus 210 estudiantes reales.")
    else:
